...
def calculate_mse(expert_trajectory, generated_trajectory):
    ...
    expert_traj = expert_trajectory[:min_len]
    generated_traj = generated_trajectory[:min_len]
    
    return np.mean((expert_traj - generated_traj) ** 2)
...
def prepare_training_data(trajectories, final_point):
    X_train = []
    Y_train = []
...
X_train, Y_train = prepare_training_data(expert_data, final_point)
X_train_rev, Y_train_rev = prepare_training_data(expert_data_rev, final_point_rev)
...
for epoch in range(num_epochs):
    ...
    if (epoch + 1) % 50 == 0:
        print(f'Epoch [{epoch + 1}/{num_epochs}], Loss: {loss.item():.4f}, Loss_rev: {loss_rev.item():.4f}')

# 生成多条轨迹用于评估
num_generated_trajectories = 100  # 与专家轨迹数量匹配
...
emd_value_up = calculate_emd(expert_data[:num_generated_trajectories], generated_trajectories_up)
emd_value_down = calculate_emd(expert_data_rev[:num_generated_trajectories], generated_trajectories_down)
