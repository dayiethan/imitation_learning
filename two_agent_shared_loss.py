...
try:
    ...
    for i in range(10000):
        ...
        all_down_points.append((20 - x, -y))  # 生成相反方向的轨迹
...
generated_trajectories_up = [
    generate_trajectory(model_up, initial_point_up, final_point_up, points_per_trajectory)
    for _ in range(num_generated)
]

# 生成下行轨迹
generated_trajectories_down = [
    generate_trajectory(model_down, initial_point_down, final_point_down, points_per_trajectory)
    for _ in range(num_generated)
]
...
emd_value_up = calculate_emd(expert_data_up[:num_generated], generated_trajectories_up)
emd_value_down = calculate_emd(expert_data_down[:num_generated], generated_trajectories_down)
...
for i, traj in enumerate(expert_data_up[:5]):
    ...
    plt.plot(x, y, 'b--', alpha=0.3, label='Expert Up' if i == 0 else "")

for i, traj in enumerate(expert_data_down[:5]):
    x = [point[0] for point in traj]
    y = [point[1] for point in traj]
    plt.plot(x, y, 'g--', alpha=0.3, label='Expert Down' if i == 0 else "")
...
plt.scatter(initial_point_up[0], initial_point_up[1], c='green', s=100, label='Start (Up)')
plt.scatter(final_point_up[0], final_point_up[1], c='red', s=100, label='End (Up)')
plt.scatter(initial_point_down[0], initial_point_down[1], c='purple', s=100, label='Start (Down)')
plt.scatter(final_point_down[0], final_point_down[1], c='orange', s=100, label='End (Down)')
...
plt.tight_layout()
try:
    plt.savefig('/mnt/data1/chendazhong/imitation_learning/figures/two_agent_shared_loss/TASM_sharedloss_with_emd.png')
except Exception as e:
    print(f"Warning: Failed to save figure: {e}. Check directory permissions.")
plt.show()
